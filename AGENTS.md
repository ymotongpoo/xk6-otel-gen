...
| 作業の性質 | 推奨エージェント | 根拠 |
|---|---|---|
| ゼロから複雑なアルゴリズムを実装 (Parse pipeline、DAG 検証等) | **Codex** | 深い reasoning、長時間バッチ |
| 既存パターンに沿って同型のコードを追加 (例: 同じスタイルの generator を 18 個追記) | **Cursor (batch)** | codebase-aware、既存規約への準拠が自然 |
| Codex 出力の review / 微調整 / typo 修正 | **Cursor (対話)** | エディタ context で diff 単位の編集が容易 |
| Lint/format/coverage 不足の補填パス | どちらでも | 機械的作業、両者ともこなせる |
| デバッグ (test 失敗の原因究明) | **Cursor (対話)** | 反復 + ファイル context の参照を要する |
| 大規模リファクタリング (パッケージ移動等) | **Codex** | 多ファイル一括変更の自律性 |

実装プラン (`code-generation-plan.md`) には、各 Phase に **推奨エージェント** が注記される。利用者は別エージェントで実行することも可能だが、推奨に従うのが効率的。

...
- **相関の取れたシグナル** — トレース・メトリクス・ログ・プロファイルを、トレースコンテキストを保ったまま生成します。メトリクスにはエグゼンプラー（`trace_id` / `span_id`）が付き、messaging には producer↔consumer のスパンリンクが張られます。
- **オペレーション単位のテレメトリ** — 構造化 `log_events`・カスタム `metrics`・Pyroscope `profile` フレームグラフをオペレーションに直接宣言でき、fault 連動の値で現実的なインシデントを再現します。

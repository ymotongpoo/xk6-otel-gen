...
func requireDocker(t *testing.T) {
	...
	if _, err := exec.LookPath("docker"); err != nil {
		t.Skipf("docker is required for k6otelgen integration tests: %v", err)
	}
}
...
func requireXK6(t *testing.T) string {
	...
	if err != nil {
		t.Skipf("xk6 is required for k6otelgen integration tests: %v", err)
	}
	return xk6
}
...
func readCollectorTraces(t *testing.T, path string) string {
	t.Helper()
	return readCollectorFile(t, path)
}
